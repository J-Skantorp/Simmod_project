...
class PPsystem:
    ...
    def __init__(self, init_cond, L, T, r=2, mod = True, g=100, b=0, a = 100):
...
def statistical_accuracy():
    time = 400
    eq = 100
    [L, T, g, b, a] = [40, time+eq, 100, 0, 100]
    runs = 50
    sims = []
    std_err_hist = [[],[],[]]
    fr_g = []
    ...
    for i in range(runs):
        ...
        PP.integrate()
        fr_g.append(sum(PP.gamma[eq:-1])/((time+1)*40*40))
        fr_b.append(sum(PP.beta[eq:-1])/((time+1)*40*40))
        fr_a.append(sum(PP.alpha[eq:-1])/((time+1)*40*40))

        var = [sum([f**2 for f in fr_g]) / len(fr_g) - (sum(fr_g) / len(fr_g))**2, sum([f ** 2 for f in fr_b]) / len(fr_g) - (sum(fr_b) / len(fr_g)) ** 2, sum([f ** 2 for f in fr_a]) / len(fr_g) - (sum(fr_a) / len(fr_g)) ** 2]
        if len(fr_g) > 2:
    ...
    plt.plot(sims, std_err_hist[0], sims, std_err_hist[2])
    plt.legend(["Population density: Gamma, mean: " + str(round(sum(fr_g)/runs)), "Population density: Alpha, mean: " + str(round(sum(fr_a)/runs))])
